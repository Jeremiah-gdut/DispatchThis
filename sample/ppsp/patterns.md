...
- Required proof: both routes require an exact current one-block boolean selector tail, selector store/load on the same direct base and exact offset, a direct-base table-pointer load, and initialized executable entries. The frozen-prefix route additionally requires exactly the modeled selector STORE after the prefix, its pointer-sized prefix value, statically addressed later STOREs disjoint from the table slot, and the frozen-prefix call/intrinsic safety policy. When those global guards are unavailable, the alternate route requires a complete singleton value for the current SSA table-pointer load and an unchanged selector base across the whole tail. The result is sorted and de-duplicated.
- Safe rejection: an available but incomplete unique route state, changed selector base within the tail, a non-singleton/incomplete SSA table-pointer value, an extra selector-slot write within the selector tail, table-slot overlap or an unknown effect on the frozen-prefix route, pointer overflow, malformed tail, unsupported predicate, or non-executable entry emits no fact.
- Test: `tests/test_branch_targets.py::test_branch_targets_recovers_a_frozen_boolean_selector_after_ambiguous_routes` and `::test_branch_targets_recovers_a_noisy_frozen_selector_from_its_ssa_table_value`, plus comparison, extra-store, duplicate-entry, and non-singleton-SSA regressions.
