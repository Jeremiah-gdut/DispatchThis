...
字符串解密和去平坦化算法不属于稳定 helper 表面。字符串恢复只由外部
`SampleSemantics.string_recovery` provider 实现；旧 profile 不再有字符串 hook。去平坦化的
`plan_deflatten_redirections` 仍只是迁移参考，MLIL 改写始终由后端执行。
...
`plan_deflatten_redirections(bv, func, mlil)` 是 bundled legacy profile 的私有迁移接口，
它仍可返回下列字典计划；核心会先将其转换为冻结的 `DeflattenPlan`，再交给原子后端：
...
外部 provider 不得返回这类字典。它的
`deflatten(DeflattenQuery)` 必须返回 `CompleteBatch[DeflattenPlan]`：无条件计划使用
`DeflattenPlanKind.UNCONDITIONAL`、精确 `owner_block`、全部 `exit_redirections` 与一个
`DeflattenStateToken`；条件计划使用 arm-exit、shared-exit 或 shortcut kind，并携带原始
`condition_il`、精确出口/共享出口、两臂 token 与目标。每个计划的
`obsolete_state_writes` 必须与其 `DeflattenStateWriteWitness` 一一对应；共享条件重放还必须
携带 `replay_condition_il` 和 `DeflattenConditionWitness`。这些对象只表达当前 IL 证据，不能
包含 `AnalysisContext`、回调或任何改写操作。
...
外部 `DeflattenPlan.obsolete_state_writes` 是 `frozenset[int]`；上面的 bundled legacy
字典计划才使用可变的 `set[int]`。两者都只包含因该计划重定向而证明冗余的精确当前 MLIL
...
外部 provider 的 `string_recovery(StringRecoveryQuery)` 返回
`CompleteBatch[StringRecoveryFact]`。Query 中的 `deflattened_function_starts` 是核心从跨函数
gate 复制出的只读快照；provider 不读取 `session_data`，只在自己的精确样本候选确实依赖时
检查该集合。工作流把事实写成幂等的函数注释。
