...
阶段依赖为：`callTargets`、`globalData` 和 `stringRecovery` 没有 DispatchThis phase 前置条件；`branchConditions` 依赖 `branchTargets`；`correlatedStores` 依赖 branch/call/global；`deflatten` 依赖 branch/call/global/branchConditions。它们仍按表中的固定 MLIL 顺序执行。`stringRecovery` 可收到为空的 `deflattened_function_starts` 快照，且不改动 cleanup receipt。菜单仅为有依赖的下游自动启用前置阶段。
