...
1. 需要恢复 CFG 的间接分支只能在 LLIL 阶段处理。
2. provider 只读取当前 Query 并返回事实或计划；workflow callback 独占 Binary Ninja mutation。
3. 后续阶段只等待自己的 gate；固定 activity 位置不等于全部阶段互相等待。
...
| 同上 | `analysis.plugins.dispatchThis.stringRecovery` | `extension.DispatchThis.StringRecovery` | MLIL | 独立扫描当前 MLIL，写字符串恢复注释。 |
| 同上 | `analysis.plugins.dispatchThis.deflatten` | `extension.DispatchThis.Deflatten` | MLIL | 原子重写已证明 dispatcher 边。 |

### Phase gate

- 不等待其他 DispatchThis phase：`callTargets`、`globalData`、`stringRecovery`。
- 只等待 branch：`branchConditions`。
- 等待 branch/call/global：`correlatedStores`。
- 等待 branch/call/global/branchConditions：`deflatten`。

它们仍按表中的固定 MLIL 顺序执行。`stringRecovery` 可收到为空的 `deflattened_function_starts` 快照，且不改动 cleanup receipt。菜单仅为有依赖的下游自动启用前置阶段。
...
1. 从 `AnalysisContext` 取得当前 IL，重新验证事实、计划和 cleanup 根。
2. `replace_expr` 后调用 `finalize()`；下一步依赖数据流时重新生成 SSA。
3. 用 `AnalysisContext.set_mlil_function(...)` 安装新的 MLIL copy-transform。
