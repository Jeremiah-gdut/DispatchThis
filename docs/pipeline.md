...
阶段依赖为：`branch → call → global → branchConditions → deflatten`，`correlatedStores` 依赖 `global`。`stringRecovery` 位于固定 MLIL 顺序中，但没有 DispatchThis phase 前置条件：它只使用自己的开关和当前 MLIL，并可收到为空的 `deflattened_function_starts` 快照；它不等待 branch/call/global/deflatten，也不改动 cleanup receipt。菜单仅为有依赖的下游自动启用前置阶段。
