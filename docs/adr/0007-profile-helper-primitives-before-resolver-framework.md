...
> 兼容策略部分已由 [ADR 0036](0036-require-exact-provider-api-version.md) 取代：外部 provider
> API 现已成为明确边界，并要求精确整数版本匹配。本文其余 helper 分层决策仍保留。
...
Profile helper API 应清晰且有文档。这些 API 可以直接暴露 Binary Ninja IL object；不要为了
隐藏 Binary Ninja API 而引入 wrapper type。其破坏性变更遵守当前核心 API 版本契约。
