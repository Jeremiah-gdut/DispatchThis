...
1. 这是核心职责，还是一个具体样本 provider 的识别规则？
2. 修改是否会触发重新分析？若会，只能在 `workflow.py` callback 边界提交。
3. 证据是否来自当前 activity 的 IL？重分析后的旧 IL、索引和 SSA 都无效。
4. 这是可复用、可测试的核心缺口，还是应留在样本插件中的受限模式？
...
2. 间接调用、全局数据、分支条件、关联 STORE、字符串和 deflatten 在 **MLIL** 中、`generateHighLevelIL` 之前按固定位置运行。间接调用、全局数据和字符串恢复都只需要自己的开关和当前 MLIL，不等待其他 DispatchThis phase；字符串恢复不改变 cleanup receipt，`deflattened_function_starts` 只是可为空的当前快照。
3. 分支条件翻译只等待 branch 恢复；关联 STORE 仍等待 branch/call/global，deflatten 仍需要 branch/call/global、当前分支条件与 cleanup 证明。固定位置不等于用户可见的 pass 依赖。
...
| 你要改什么 | 先读 |
| --- | --- |
| 新样本的识别或解码 | [docs/sample-providers.md](docs/sample-providers.md) 与 [docs/obfuscation.md](docs/obfuscation.md) |
| workflow、receipt、当前 IL 或 cleanup | [docs/pipeline.md](docs/pipeline.md) 与相关 ADR |
| provider 的 Query、事实或计划 | [docs/API.md](docs/API.md) |
| dispatcher/条件去平坦化 | [docs/conditional-deflattening.md](docs/conditional-deflattening.md) 与相关 ADR |
