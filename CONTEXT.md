...
2. 间接调用、全局数据、分支条件、关联 STORE、字符串和 deflatten 在 **MLIL** 中、`generateHighLevelIL` 之前按固定位置运行。字符串恢复只需要自己的开关和当前 MLIL，不等待 branch/call/global/deflatten，也不改变 cleanup receipt；`deflattened_function_starts` 只是可为空的当前快照。
3. 分支和调用稳定后才允许其真实下游消费它们；全局数据在分支条件之前；deflatten 需要当前分支条件与 cleanup 证明。固定位置不等于用户可见的 pass 依赖。
